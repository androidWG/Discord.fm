...
class AppManager:
    def __init__(self):
        self.tray_icon = system_tray_icon.SystemTrayIcon(self.close_app)
        self.loop = loop_handler.LoopHandler(self.tray_icon)

    def start(self):
        atexit.register(globals.manager.close_app)
        ...
        try:
            ...
            if not sc.empty():
                ...
                for event in sc.queue:
                    sc.cancel(event)
                    logging.debug(f"Event of class \"{event.__class__}\" closed")
