...
if opts.requirements_file:
    ...
    exit('Please specify requirements file argument')
...
if opts.output:
    ...
    output_package = 'python3-{}'.format(os.path.basename(opts.requirements_file).replace('.txt', ''),
    )
elif len(packages) == 1:
    output_package = 'python3-{}'.format(packages[0].name,)
else:
    output_package = 'python3-modules'
...
system_packages = ['cython', 'easy_install', 'mako', 'markdown', 'meson', 'pip', 'pygments', 'setuptools']
